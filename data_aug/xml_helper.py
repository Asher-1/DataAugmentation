...
def generate_xml(img_name, coords, img_size, out_root_path):
    ...
    title_text = doc.createTextNode('Annotations')
    ...
    for coord in coords:
        ...
        title_text = doc.createTextNode(str(max(int(float(coord[0])), 0)))
        title.appendChild(title_text)
        bndbox.appendChild(title)
        title = doc.createElement('ymin')
        title_text = doc.createTextNode(str(max(int(float(coord[1])), 0)))
        title.appendChild(title_text)
        bndbox.appendChild(title)
        title = doc.createElement('xmax')
        title_text = doc.createTextNode(str(min(int(float(coord[2])), int(img_size[1]) - 1)))
        title.appendChild(title_text)
        bndbox.appendChild(title)
        title = doc.createElement('ymax')
        title_text = doc.createTextNode(str(min(int(float(coord[3])), int(img_size[0]) - 1)))
        title.appendChild(title_text)
        bndbox.appendChild(title)
